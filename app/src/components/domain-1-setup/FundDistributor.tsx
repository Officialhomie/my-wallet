...
export function FundDistributor() {
  ...
  const [verificationTokenType, setVerificationTokenType] = useState<'native' | 'erc20'>('native');
  ...
  const handleVerifyBalances = async () => {
    try {
      // Use appropriate min balance based on token type (USDC typically needs different precision)
      const minBalance = verificationTokenType === 'erc20' ? '0.01' : '0.0001';
      const result = await verifyBalances(selectedNetwork, minBalance, verificationTokenType);
      // Store the token type in the result for display purposes
      setVerificationResult({
        ...result,
        _verificationTokenType: verificationTokenType // Store the token type we used
      });
    } catch (error: any) {
      setVerificationResult({ 
        error: error.message || 'Failed to verify balances',
        details: error.message,
        _verificationTokenType: verificationTokenType
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-foreground">Tvrch Fund Distribution</h3>
            ...
            <form onSubmit={handleConfigureFundingWallet} className="space-y-4 bg-card rounded-md border border-border p-4 sm:p-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Select the network for your funding wallet. The wallet can distribute to any network, but must have funds on the selected network.
                </p>
              </div>
        ...
        <section className="bg-card rounded-md border border-border p-4 sm:p-6">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button
                onClick={() => {
                  setShowFundingWalletForm(true);
                  setPrivateKey('');
                  setFundingNetwork(fundingWallet.network || selectedNetwork);
                }}
                variant="secondary"
                size="sm"
              >
            ...
            <div className="bg-muted/50 rounded-md p-3 space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{fundingWallet.network || 'Not set'}</span>
              </div>
            </div>
            
            {/* Show reconfigure form if user clicked Reconfigure */}
            {showFundingWalletForm && (
              <form onSubmit={handleConfigureFundingWallet} className="mt-4 space-y-4 bg-muted/30 rounded-md border border-border p-4">
                <Input
                  label="Private Key"
                  type="password"
                  value={privateKey}
                ...
                <div className="flex gap-2">
                  <Button type="submit" variant="primary" disabled={isConfiguringWallet}>
                    {isConfiguringWallet ? 'Configuring...' : 'Update Configuration'}
                  </Button>
                  <Button 
                    type="button" 
                    variant="secondary" 
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => {
                  setVerificationTokenType('native');
                  setVerificationResult(null); // Clear previous results when switching
                }}
                className={`flex-1 px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                  verificationTokenType === 'native'
                    ? 'bg-primary text-primary-foreground'
                    : 'bg-muted text-muted-foreground hover:bg-muted/80'
                }`}
              >
              ...
              <button
                type="button"
                onClick={() => {
                  setVerificationTokenType('erc20');
                  setVerificationResult(null); // Clear previous results when switching
                }}
                className={`flex-1 px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                  verificationTokenType === 'erc20'
                    ? 'bg-primary text-primary-foreground'
                    : 'bg-muted text-muted-foreground hover:bg-muted/80'
                }`}
              >
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Select which token type to verify balances for
            </p>
          </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              >
                Verify {verificationTokenType === 'native' ? 'ETH' : 'USDC'} Balances
              </Button>
            </div>

            {fundingWallet?.configured && fundingWallet.network && fundingWallet.network !== selectedNetwork && (
              <div className="bg-warning/10 border border-warning rounded-md p-3 text-sm text-warning-foreground">
                ...
                The system will automatically use the correct network provider for distribution.
              </div>
            )}
        </section>
      )}
      ...
      {verificationResult && (
        <section className="bg-card border border-border py-3 px-6 space-y-2 rounded-[5px]">
          ...
          {verificationResult.error ? (
            ...
            <div className="space-y-2">
              ...
              {verificationResult.insufficientWallets && verificationResult.insufficientWallets.length > 0 && (
                <div className="bg-warning/10 border border-warning rounded-md p-3 rounded-[5px] py-[3px] px-[6px]">
                  ...
                  <div className="space-y-1 text-xs">
                    {verificationResult.insufficientWallets.slice(0, 5).map((w: any, i: number) => {
                      // Determine if this is USDC or ETH based on result tokenType or stored verification type
                      const isUSDC = verificationResult.tokenType === 'ERC20' || 
                                     verificationResult.tokenType === 'erc20' || 
                                     verificationResult._verificationTokenType === 'erc20' ||
                                     w.tokenType === 'ERC20';
                      
                      // Format balance appropriately (USDC uses 2 decimals, ETH uses 6)
                      const balanceDisplay = isUSDC 
                        ? (w.currentBalance?.toFixed(2) || '0.00')
                        : (w.currentBalance?.toFixed(6) || '0.000000');
                      
                      // Format required amount appropriately
                      const requiredDisplay = isUSDC
                        ? parseFloat(w.required).toFixed(2)
                        : parseFloat(w.required).toFixed(6);
                      ...
                      return (
                        <div key={i} className="flex justify-between">
                          ...
                          <span>{balanceDisplay} / {requiredDisplay} {tokenSymbol}</span>
                        </div>
                      );
                    })}
                  ...
                  <p className="text-sm font-medium text-success">
                    ✅ All wallets have sufficient balance ({(() => {
                      const isUSDC = verificationResult.tokenType === 'ERC20' || 
                                     verificationResult.tokenType === 'erc20' || 
                                     verificationResult._verificationTokenType === 'erc20';
                      const formattedMin = isUSDC 
                        ? parseFloat(verificationResult.minimumRequired).toFixed(2)
                        : parseFloat(verificationResult.minimumRequired).toFixed(6);
                      return `${formattedMin} ${isUSDC ? 'USDC' : 'ETH'}`;
                    })()} minimum)
                  </p>
                </div>
              )}
