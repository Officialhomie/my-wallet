...
import { selectWalletActivityMetrics } from '@/store/selectors/walletSelectors';
...
export function WalletGrid({ className = '' }: WalletGridProps) {
  const { wallets, totalWallets } = useStore((state) => state.walletActivity);
  const { activeWallets, errorWallets, globalStats } = useStore(selectWalletActivityMetrics);
