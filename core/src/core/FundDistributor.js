...
export class FundDistributor {
  ...
  constructor(walletFarm, options = {}) {
    ...
    this.fundingWalletNetwork = null; // Track which network the funding wallet was configured for
   ...
   * @param {string} [network] - Network name the wallet is configured for
   */
  setFundingWallet(wallet, network = null) {
    ...
    this.fundingWalletNetwork = network;

    if (this.options.verbose) {
      console.log(chalk.green(`✅ Funding wallet set: ${wallet.address}${network ? ` (network: ${network})` : ''}`));
    }
  }

  /**
   * Gets the funding wallet's configured network
   * @returns {string|null} Network name or null
   */
  getFundingWalletNetwork() {
    return this.fundingWalletNetwork;
  }

  /**
   * Creates a provider for the target network and returns a wallet connected to it
   * This allows the funding wallet to work across different networks
   *
   * @param {string} targetNetwork - Target network name
   * @param {Object} rpcUrls - Map of network names to RPC URLs
   * @returns {ethers.Wallet} Wallet connected to target network
   */
  getFundingWalletForNetwork(targetNetwork, rpcUrls) {
    if (!this.fundingWallet) {
      throw new Error('Funding wallet not configured. Please configure a funding wallet first.');
    }

    if (!rpcUrls[targetNetwork]) {
      throw new Error(`Unsupported network: ${targetNetwork}. Supported networks: ${Object.keys(rpcUrls).join(', ')}`);
    }

    // Create a new provider for the target network
    const provider = new ethers.JsonRpcProvider(rpcUrls[targetNetwork]);
    // Return a new wallet instance connected to the target network (same private key)
    return new ethers.Wallet(this.fundingWallet.privateKey, provider);
  }
