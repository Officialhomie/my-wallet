...
export class UserArchetypes {
  constructor(options = {}) {
    ...
    this.archetypes = {
      ...
      activeTrader: {
        ...
        contractInteractions: {
          preferredFunctions: ['any'], // ✅ Made more permissive - accepts any function
          avoidFunctions: [], // No restrictions
      ...
      flexible: {
        name: 'flexible',
        description: 'Universal user that can interact with any contract function',
        frequency: 'high',
        volume: 'medium',
        delayMin: 10000,  // 10 seconds
        delayMax: 120000, // 2 minutes
        transactionSize: { min: 0.01, max: 5 },
        skipProbability: 0.1, // 10% chance to skip
        burstBehavior: {
          enabled: true,
          burstSize: { min: 2, max: 5 },
          burstFrequency: 0.3, // 30% chance of burst behavior
          interBurstDelay: 'normal'
        },
        contractInteractions: {
          preferredFunctions: ['any'], // Accepts ANY function
          avoidFunctions: [], // No restrictions
          parameterVariation: 'medium'
        },
        timingPatterns: {
  ...
  isFunctionSuitable(archetypeName, functionName) {
    ...
    // Check if function matches preferred patterns (e.g., 'claimFaucet' matches 'claim')
    const matchesPreferred = interactions.preferredFunctions.some(pref => {
      if (pref === 'any') return true;
      // Exact match
      if (functionName === pref) return true;
      // Pattern match: functionName starts with preferred (e.g., 'claimFaucet' starts with 'claim')
      if (functionName.toLowerCase().startsWith(pref.toLowerCase())) return true;
      // Pattern match: preferred is a substring (e.g., 'transfer' in 'transferFrom')
      if (functionName.toLowerCase().includes(pref.toLowerCase())) return true;
      return false;
    });
    if (matchesPreferred) {
      return true;
    ...
    // Check if should avoid (exact match or starts with)
    const shouldAvoid = interactions.avoidFunctions.some(avoid => {
      if (functionName === avoid) return true;
      if (functionName.toLowerCase().startsWith(avoid.toLowerCase())) return true;
      return false;
    });
    if (shouldAvoid) {
      return false;
    ...
    // Default to allowing common function patterns
    const commonPatterns = ['transfer', 'swap', 'deposit', 'withdraw', 'claim', 'stake', 'mint', 'burn', 'approve'];
